			...
			while(true){
				...
				if(incomingPacket.getLength() >50){
					...
					if("operation".equalsIgnoreCase(msg.getMessageType())){
						...
						new MainServer().setFrontEndAddress(msg.getMessage());
						...
						socket2.send(packet);
						System.out.println("response sent from Replica3"+message.getMessageID());
					}	
					...
					{	System.out.println("Request for BackUp");
						...
						System.out.println("Sending BackUp with size : "+new ReplicaManager().getRequestHandler().size());
					 ...
					 if(request.startsWith("FD")){
						     String reply = ""+configuration.PROCESS_ID;
                    	 ...
                    	 new MainServer().setGroupLeaderProcessID(Integer.parseInt(request.split(":")[1].trim()));
                     }
				}
			}
		}catch(Exception e){
