...
public class MainServer extends DFRSInterfacePOA implements Runnable{
	...
	private static ArrayList<MainServer> DFRSServers = new ArrayList<MainServer>();
	...
	public static MainServer mtlServer = null;
	public static MainServer wstServer = null;
	public static MainServer ndlServer = null;
	...
	private void setLogger(String serverName) {
		String logFile = configuration.SERVER_LOG_PATH + serverName + configuration.LOG_EXTENSION;
	 ...
	 * @param args
	 */
	public MainServer(String serverName, int udpPort, String serverAddress) {
	...
	public static void main(String[] args) {
		try{
			 mtlServer = new MainServer(configuration.RMI_SERVER_1_NAME, configuration.UDP_PORT_1_NUMBER,
					configuration.RMI_SERVER_1_ADDRESS);
			 wstServer = new MainServer(configuration.RMI_SERVER_2_NAME, configuration.UDP_PORT_2_NUMBER,
					configuration.RMI_SERVER_2_ADDRESS);
			 ndlServer = new MainServer(configuration.RMI_SERVER_3_NAME, configuration.UDP_PORT_3_NUMBER,
					configuration.RMI_SERVER_3_ADDRESS);

			mtlServer.setNumberOfFlights(configuration.MTL_NUMBEROFFLIGHT);
			wstServer.setNumberOfFlights(configuration.WST_NUMBEROFFLIGHT);
			ndlServer.setNumberOfFlights(configuration.NDL_NUMBEROFFLIGHT);
			
			mtlServer.setEconomy_numberOfSeats(configuration.ECONOMY_NUMBEROFSEATS);
			wstServer.setEconomy_numberOfSeats(configuration.ECONOMY_NUMBEROFSEATS);
			ndlServer.setEconomy_numberOfSeats(configuration.ECONOMY_NUMBEROFSEATS);

			mtlServer.setBuisness_numberOfSeats(configuration.BUISNESS_NUMBEROFSEATS);
			wstServer.setBuisness_numberOfSeats(configuration.BUISNESS_NUMBEROFSEATS);
			ndlServer.setBuisness_numberOfSeats(configuration.BUISNESS_NUMBEROFSEATS);
			
			mtlServer.setFirst_numberOfSeats(configuration.FIRST_NUMBEROFSEATS);
			wstServer.setFirst_numberOfSeats(configuration.FIRST_NUMBEROFSEATS);
			ndlServer.setFirst_numberOfSeats(configuration.FIRST_NUMBEROFSEATS);
			...
			POA rootPOA = null;
			Iterator<MainServer> DFRSIterator = DFRSServers.iterator();
			while (DFRSIterator.hasNext()) {
				try {
					MainServer dfrsServer = DFRSIterator.next();
					...
					PrintWriter file = new PrintWriter(configuration.IOR_PATH+dfrsServer.getServerName()+configuration.LOG_EXTENSION);
					file.println(ior);
					file.close();
				} catch (InvalidName e) {
			...
			try{
				...
				new MulticastMessageReceiver(configuration.MC_OPERATION_PORT);
	...
	public PassengerVO bookFlight(String firstName, String lastName, String address, String phoneNumber,
		...
		if(this.recordMap.get(lastName.charAt(0))== null){
			...
			for (int i = 0; i < temperory.size(); i++) {
				...
				if(passengerObj.firstName.equalsIgnoreCase(passengerVO.firstName) && passengerObj.firstName.equalsIgnoreCase(passengerVO.lastName)){
					...
					passengerVO.message = (configuration.PASSENGER_EXISTS);
					logger.info("Failure : "+configuration.PASSENGER_EXISTS + "with FIRST NAME: " + firstName + " and LAST NAME: " + lastName);
					return passengerVO;
				}
			}
		}
		...
		for (Iterator iterator = listOfFlight.iterator(); iterator.hasNext();) {
			...
			{	
				if(passengerVO.classOfFlight.equalsIgnoreCase(configuration.BUISNESS_CLASS)){
					seat = flightVO.businessSeats;
					if(seat!=0)
					flightVO.businessSeats = seat - 1;
				}
				else if(passengerVO.classOfFlight.equalsIgnoreCase(configuration.ECONOMY_CLASS)){
				...
				if(seat<=0){
					passengerVO.message = ("Seats are not avilable");
					passengerVO.isFlightBooked = false;
					logger.info("Seats are not avilable");
					return passengerVO;
				}
				else{
				passengerVO.message = ("SUCCESS: "+configuration.PASSENGER_SUCCESS+ "RecordID : "+passengerVO.recordID);
				logger.info(configuration.PASSENGER_SUCCESS +"\n============================================================\nPassenger Details:\n============================================================" + "\n\tRecordId:\t" + passengerVO.recordID + "\n\tFirst Name:\t" + passengerVO.firstName+ "\n\tLast Name:\t" + passengerVO.lastName + "\n\tAddress:\t" + passengerVO.address + "\n\tPhone Number:\t"
	...
	private static void updatePassengerRecords(MainServer dfrsServer) {
		BufferedWriter out = null;
		try  
		{
		    FileWriter fstream = new FileWriter(configuration.PASSENGER_RECORD_OUTPUT+ "-" + dfrsServer.serverName + configuration.LOG_EXTENSION, false); //true tells to append data.
	...
	private static void updateFLightRecords() {
		...
		try  
		{
		    FileWriter fstream = new FileWriter(configuration.FLIGHT_RECORD_OUTPUT + configuration.LOG_EXTENSION, false); //true tells to append data.
		    out = new BufferedWriter(fstream);
	...
	private static void initiateFlightRecords() {
		...
		Iterator<MainServer> dFRSIterator = DFRSServers.iterator();
		while (dFRSIterator.hasNext()) {
			MainServer dfrsServer = (MainServer) dFRSIterator.next();
			ArrayList<FlightVO> temporary = new ArrayList<FlightVO>();
			j=0;
			while(j<3){i=1;
			while(i<=configuration.BOOKING_DATE_LIMIT){
				if(i==1)
				...
				flightVO.economySeats = (configuration.ECONOMY_NUMBEROFSEATS);
				flightVO.businessSeats = (configuration.BUISNESS_NUMBEROFSEATS);
				flightVO.firstClassSeats = (configuration.FIRST_NUMBEROFSEATS);
				flightVO.editedBy = ("NONE");
				if(dfrsServer.serverName.equalsIgnoreCase(configuration.RMI_SERVER_1_NAME))
				{
					flightVO.Destination = (configuration.RMI_SERVER_2_NAME);
				}else if(dfrsServer.serverName.equalsIgnoreCase(configuration.RMI_SERVER_2_NAME))
				{
					flightVO.Destination = (configuration.RMI_SERVER_1_NAME);
				}else if(dfrsServer.serverName.equalsIgnoreCase(configuration.RMI_SERVER_3_NAME))
				{
					flightVO.Destination = (configuration.RMI_SERVER_2_NAME);
				}
			...
			while(i<=configuration.BOOKING_DATE_LIMIT){
				...
				if(dfrsServer.serverName.equalsIgnoreCase(configuration.RMI_SERVER_1_NAME))
				{
					flightVO.Destination = (configuration.RMI_SERVER_3_NAME);
				}else if(dfrsServer.serverName.equalsIgnoreCase(configuration.RMI_SERVER_2_NAME))
				{
					flightVO.Destination = (configuration.RMI_SERVER_3_NAME);
				}else if(dfrsServer.serverName.equalsIgnoreCase(configuration.RMI_SERVER_3_NAME))
	...
	public int[] getBookedFlightCount(String recordType) {
		...
		passengerWrapperVO.setRequestType(configuration.REQUEST_TYPE.FLIGHT_COUNT);
	...
	public FlightVO editFlightRecord(String recordID, String fieldName, String newValue) {
		...
		if(data[1].substring(0, 3).equalsIgnoreCase("MTL")) 
			listFlight = flightRecordMap.get(configuration.RMI_SERVER_1_NAME);
		if(data[1].substring(0, 3).equalsIgnoreCase("WST"))
			listFlight = flightRecordMap.get(configuration.RMI_SERVER_2_NAME);
		if(data[1].substring(0, 3).equalsIgnoreCase("NDL"))
			listFlight = flightRecordMap.get(configuration.RMI_SERVER_3_NAME);
		...
		if( available){
			...
			if(fieldName.equalsIgnoreCase(configuration.DEPARTURE)){
				flightVO.Departure = (newValue);
			}
			else if(fieldName.equalsIgnoreCase(configuration.DESTINATION)){
				flightVO.Destination = (newValue);
			}
			else if(fieldName.equalsIgnoreCase(configuration.BUSINESSSEATS)){
				flightVO.businessSeats = (Integer.parseInt(newValue));
			}
			else if(fieldName.equalsIgnoreCase(configuration.ECONOMYSEATS)){
				flightVO.economySeats = (Integer.parseInt(newValue));
			}
			else if(fieldName.equalsIgnoreCase(configuration.FIRSTCLASSSEATS)){
				flightVO.firstClassSeats = (Integer.parseInt(newValue));
			}
			else if(fieldName.equalsIgnoreCase(configuration.AVAILABILITY)){
			...
			else if(fieldName.equalsIgnoreCase(configuration.FLIGHTDATE)){
			...
			else if(fieldName.equalsIgnoreCase(configuration.FLIGHTTIME)){
			...
			if(data[1].startsWith("MTL")){
				flightVO.Departure = (configuration.RMI_SERVER_1_NAME);
				flightVO.Destination = (configuration.RMI_SERVER_2_NAME);
			}else if(data[1].startsWith("NDL"))
			{
				flightVO.Departure = (configuration.RMI_SERVER_3_NAME);
				flightVO.Destination = (configuration.RMI_SERVER_2_NAME);
			}
			else  if(data[1].startsWith("WST")){
				flightVO.Departure = (configuration.RMI_SERVER_2_NAME);
				flightVO.Destination = (configuration.RMI_SERVER_1_NAME);
			}
			if(fieldName.equalsIgnoreCase(configuration.AVAILABILITY)){
				if(newValue.equalsIgnoreCase("true"))
	...
	private MainServer getServerReference(String otherCity) {
		if(otherCity.equalsIgnoreCase(configuration.RMI_SERVER_1_NAME))
			return mtlServer;
		if(otherCity.equalsIgnoreCase(configuration.RMI_SERVER_2_NAME))
			return wstServer;
		if(otherCity.equalsIgnoreCase(configuration.RMI_SERVER_3_NAME))
			return ndlServer;		
	return null;
	}
	@Override
	...
	public PassengerVO transferReservation(String passengerID, String currentCity, String otherCity) {
		...
		boolean isAvailable = false;
		 for (Entry<Character, ArrayList<PassengerVO>> entry : this.recordMap.entrySet()) { 
		    	...
		    	for (Iterator iterator = temp.iterator(); iterator.hasNext();) {
					...
					if(passenger.recordID.equalsIgnoreCase(passengerID)){
						passengerWrapperVO.setRequestType(configuration.REQUEST_TYPE.TRANSFER_RESERVATION);
		 ...
		 if(isAvailable){	
			...
			MainServer dfrsServer = getServerReference(otherCity);
	 ...
	 private void rollbackDeletefromCurrentServer(MainServer dfrsServer, PassengerWrapperVO passengerWrapperVO) {
	            ...
	            if(this.recordMap.containsKey(key)){
	                ...
	                synchronized(temp){
	                    if (temp!= null){
	                        ...
	                        passengerWrapperVO.setPassengerStatus(configuration.PASSENGER_STATUS.CREATED);
	                    }
	                }
	            }else{
	...
	private PassengerWrapperVO rollBackPassenger(MainServer dfrsServer, PassengerWrapperVO passengerWrapperVO) {
			list = new ArrayList<PassengerWrapperVO>();
			passengerWrapperVO.setRequestType(configuration.REQUEST_TYPE.ROLLBACK_RESERVATION);
			try{
			ByteArrayOutputStream baos = new ByteArrayOutputStream(5000);
			ObjectOutputStream objectOS;
			objectOS = new ObjectOutputStream(new BufferedOutputStream(baos));
	...
	private PassengerWrapperVO deletefromCurrentServer(PassengerWrapperVO passengerWrapperVO, String passengerID) {
		...
		if (this.recordMap.get(passengerWrapperVO.getPassengerVO().lastName.charAt(0))!=null && this.recordMap.get(passengerWrapperVO.getPassengerVO().lastName.charAt(0)).contains(passengerVO)) {
			passengerWrapperVO.setDeletePassengerStatus(configuration.PASSENGER_STATUS.ERROR);
		} else {
			passengerWrapperVO.setDeletePassengerStatus(configuration.PASSENGER_STATUS.DELETED);
		}
		updatePassengerRecords(this);
	...
	private PassengerWrapperVO sendTransferReservationRequest(MainServer dfrsServer,
	...
	public PassengerWrapperVO receivePassengerReservation(PassengerWrapperVO passengerWrapperVO) {
		...
		if(available){
			...
			passengerWrapperVO.setPassengerStatus(configuration.PASSENGER_STATUS.ERROR);
	...
	public PassengerWrapperVO deleteReservation(PassengerWrapperVO passengerWrapperVO) {
		...
		if (this.recordMap.get(passengerWrapperVO.getPassengerVO().lastName.charAt(0))!=null && this.recordMap.get(passengerWrapperVO.getPassengerVO().lastName.charAt(0)).contains(ps)) {
			passengerWrapperVO.setDeletePassengerStatus(configuration.PASSENGER_STATUS.ROLLBACK_ERROR);
		} else {
			passengerWrapperVO.setDeletePassengerStatus(configuration.PASSENGER_STATUS.ROLLED_BACK);
		}
