			...
			while(true){
				...
				if(incomingPacket.getLength() >50){
					...
					if("operation".equalsIgnoreCase(msg.getMessageType())){
						...
						new DFRSServer().setFrontEndAddress(msg.getMessage());
						...
						socket2.send(packet);
						System.out.println("response sent from Replica1 : "+message.getMessageID());
					}
					else if("backup".equalsIgnoreCase(msg.getMessageType()))
					{	
						DatagramSocket socket2 = new DatagramSocket();
						ByteArrayOutputStream bos = new ByteArrayOutputStream();
						ObjectOutput oo = new ObjectOutputStream(bos);System.out.println("Requesthandler sixze"+new ReplicaManager().getRequestHandler().size());
					 ...
					 if(request.startsWith("FD")){
                    	 ...
                    	 new DFRSServer().setGroupLeaderProcessID(Integer.parseInt(request.split(":")[1].trim()));
                     }
				}
			}
		}catch(Exception e){
