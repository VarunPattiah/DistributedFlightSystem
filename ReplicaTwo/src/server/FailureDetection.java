...
public class FailureDetection extends Thread {
	...
	@Override
	public void run() {
		new MulticastMessageReceiver(Constants.MC_FD_PORT1);
		new MulticastMessageReceiver(Constants.MC_FD_PORT3);
		while(true){
			List<String> activeProcesses = new MulticastMessageSender().multicastRequest("FD", Constants.MC_FD_PORT2);
			int currentGroupLeader = new MainServerProject().getGroupLeaderProcessID();
			...
			if (activeProcesses.size() == 0) {
				...
				else if(currentGroupLeader == Constants.PROCESS_ID){
					new MulticastMessageSender().multicastRequest("LE:"+Constants.PROCESS_ID, Constants.MC_FD_PORT2);
					try {
	...
	private void notifyFrontEnd(String address) {
		...
		try{
			...
			packet = new DatagramPacket(bytes, bytes.length, InetAddress.getByName(new MainServerProject().getFrontEndAddress()), Constants.FE_NOTIFY_PORT);
			socket.send(packet);
		}catch(Exception e){
