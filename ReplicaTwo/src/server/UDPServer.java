...
public class UDPServer extends Thread {
	MainServerProject dfrsServer;

	public UDPServer(MainServerProject dfrsServer) {
	...
	public void run() {
		...
		try {
			...
			while (true) {
				...
				if (passengerWrapperVO.getRequestType() == Constants.REQUEST_TYPE.FLIGHT_COUNT) {
					...
					objectOS.flush();
				} else if (passengerWrapperVO.getRequestType() == Constants.REQUEST_TYPE.TRANSFER_RESERVATION) {
					passengerWrapperVO = dfrsServer.receivePassengerReservation(passengerWrapperVO);
					objectOS.flush();
					objectOS.writeObject(passengerWrapperVO);
					objectOS.flush();
				} else if (passengerWrapperVO.getRequestType() == Constants.REQUEST_TYPE.ROLLBACK_RESERVATION) {
